...
@router.post("", response_class=PositionResponse)
...
@router.get("", response_class=list[PositionResponse])
async def list_positions_router(db: Session = Depends(get_db)) -> list[PositionResponse]:
    return list_positions(db)

@router.get("/{book_id}/positions", response_class=PositionResponse)
